...
static int THPVariable_clear(THPVariable *self)
{
  ...
  if (self->cdata) {
    {
      std::lock_guard<std::mutex> lock(self->cdata->grad_accumulator_lock);
...
int THPVariable_set_grad(THPVariable *self, PyObject *other)
{
  ...
  {
    std::lock_guard<std::mutex> lock(var.grad_accumulator_lock);
    var.grad = other_var;
    if (auto grad_acc = var.grad_accumulator.lock()) {
      dynamic_cast<torch::autograd::AccumulateGrad*>(grad_acc.get())->variable_grad = other_var;
    }
  }
  return 0;
}
